...
class Settings(BaseSettings):
    ...
    # ZH: 公告附件的上限（擁有者裁定 2026-08-30）。兩個都可以在管理端的
    #     系統設定裡改，這裡只是 .env 沒設時的預設值。
    # ZH: 總量是**全站的**，不是每人的 —— 附件是管理員放的，
    #     算到學生的 2 GB 個人配額上是錯的。
    ANNOUNCEMENT_FILE_MAX_MB: int = 20
    ANNOUNCEMENT_FILES_TOTAL_GB: int = 2
