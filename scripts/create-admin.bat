...
docker exec -e ADMIN_USER=%ADMIN_USER% -e ADMIN_EMAIL=%ADMIN_EMAIL% -e ADMIN_PW=%ADMIN_PW% %CONTAINER% python -c "import os; from app.database import SessionLocal; from app import models as m, crud as c; from app.config import settings as s; db=SessionLocal(); U=os.environ['ADMIN_USER']; P=os.environ['ADMIN_PW']; E=os.environ.get('ADMIN_EMAIL','admin@local'); u=c.get_user_by_username(db,U) or m.User(username=U,email=E,role='admin',is_admin=1,is_active=1); new=(u.id is None); u.hashed_password=c.get_password_hash(P); u.role='admin'; u.is_admin=1; u.is_active=1; db.add(u); db.commit(); db.refresh(u); (db.query(m.TokenUsage).filter(m.TokenUsage.user_id==u.id).first() or (db.add(m.TokenUsage(user_id=u.id,tokens_used=0,tokens_limit=s.DEFAULT_MONTHLY_TOKEN_LIMIT,reset_date=c._calculate_next_reset_date())), db.commit())); print('CREATED' if new else 'UPDATED', 'admin:', U)"
