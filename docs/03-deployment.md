...
**admin (:8888) 可跨機登入**（走本機帳密、不經 SSO）。~~但目前是 http~~ —— **2026-09-01 起已強制 TLS**（`listen 8888 ssl`，與使用者端同一張 LE 憑證）。⚠ 但它**對外開著**，見 `14-正式環境建置.md` §6.1，
所以~~管理員密碼會明文經過網路~~**已不成立**（HTTPS 已上線）。現在真正的風險不是竊聽，而是**這個管理端在公網上找得到**。
