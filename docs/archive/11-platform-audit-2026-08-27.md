...
### 問題 10 — 🔴 小基答得頭頭是道但內容是編的（已修，根因是嵌入模型）

**症狀**（2026-08-28 實測）：問「怎麼提交訓練？」，小基回答
「前往『Compute Tasks（運算任務）』分頁 → 點『New Contract』→ 選『Training Task』→ 按『Submit』」。
**那四個名稱在現在的介面上一個都不存在**，使用者照著找會找不到。
再問「文件庫有哪些分類？」，它把知識庫裡唯一那句
「Document Library（文件庫）：平台文件與資源」**拆成兩個不存在的分類**。

**第一層原因：知識庫過時。** 8 個 .md 停在 2026-06-17／25，描述的是舊的 V0 介面
（側邊抽屜、Compute Tasks、Notebook）。`Compute Tasks` 出現 9 次、`Notebook` 12 次、
`AI Base Lab` 8 次，而這些字串在現在的前端是 **0 個檔**。
反過來 `使用量`、`程式實驗室` 在知識庫裡是 **0 筆**。

→ 全部重寫成 9 份對齊 V1 的說明（`8b0c7d8`）。小標一律寫成「使用者會問的那句話」，
因為切塊器會把標題併進向量。索引重建：9 檔 83 塊。

**但只修好一半。** 重寫後「文件庫有哪些分類？」答對了，
「怎麼提交訓練？」**照樣答錯**，仍然講 Compute Tasks。查證：

- 索引裡含 `Compute Tasks` 的塊 = **0**（不是舊索引沒清）。
- 那題檢索回來的四塊**全部不相關**（問題回報、扣不扣點數、關閉實驗室），
  `30-訓練模型.md` 一塊都沒進來 → 模型拿到不相關的上下文就自己補了。

**第二層原因（真正的根因）：`nomic-embed-text` 是英文優先的嵌入模型。**
受控實驗 —— 同一個模型、同一組「問題／正確答案／不相關句子」，只換語言：

| 語言 | 正確答案 | 不相關句子 | 鑑別差距 |
|---|---|---|---|
| English | 0.759 | 0.409 | **+0.350** |
| 繁體中文 | 0.664 | 0.598 | **+0.066** |

繁中的鑑別力只有英文的**五分之一**。所有中文句子都擠在 0.6–0.8 的窄帶裡，
排序接近隨機 —— 所以「我想訓練一個模型要從哪裡開始」在知識庫裡有**逐字相同的標題**，
照樣排不到第一。**知識庫寫得再好都救不了。**

**修法**：`RAG_EMBED_MODEL` 換成多語系的 **`bge-m3`**（1024 維，約 1.2 GB），reindex。

**驗收**（12 題檢索基準，每題預先寫死「正確答案應該來自哪一個檔」）：

| | Top-1 命中 | Top-4 命中 |
|---|---|---|
| nomic-embed-text | 3/12（**25%**） | 7/12（58%） |
| bge-m3 | 12/12（**100%**） | 12/12（100%） |

端對端複測「怎麼提交訓練？」→ 回答與知識庫逐項相符（交給平台訓練頁、zip、
預設 10 輪、開始訓練、訓練紀錄），舊名稱 **0 個**。

⚠️ **部署影響**：`ollama pull bge-m3` 成為必要步驟。已更新
`docs/00-本機完整部署指南.md`（兩處）、`.env`、`.env.example`、`config.py` 預設值。
**換這個值一定要跟著 reindex** —— 維度 768→1024，舊向量不能混用。

⚠️ **新增守衛 `check_ollama_models`**：`deploy_check.py` 原本只檢查 11434 埠通不通，
那只證明 Ollama 活著、不證明它有那個模型。**模型沒下載時小基不會報錯** ——
它拿到空的檢索結果照樣回答。新的檢查會比對 `.env` 指名的模型與 `ollama list`。
陽性對照：把 `.env` 改成不存在的模型 → 由 PASS 轉 FAIL。

⚠️ **還沒解決：簡體字。** 回覆裡仍會混入「结果」「帮助」這種簡體字
（提示詞已寫「用繁體中文」，qwen2.5:7b 壓不住）。換模型後已明顯減少
（從整段夾雜變成零星幾個字），但沒有根治。擁有者尚未決定要不要加後處理轉換。

---

### 問題 11 — 🔴 國際學生問英文，小基用中文回答（未解，需要擁有者決定）

**來歷**：擁有者提出「有國際學生，可能會用英文或其他語言」。
這也讓「一律把回覆轉成繁體」那個方案直接出局 —— 那會把英文回答弄壞。

**好消息：不需要再寫一套英文知識庫。** `bge-m3` 的跨語言檢索是滿分 ——
用英文問句打**中文**知識庫，同一份 12 題基準 **Top-1 12/12（100%）**。

**壞消息：`qwen2.5:7b` 壓不住回答語言。**

原本提示詞寫死「用繁體中文」，所以英文提問也會得到中文回答。
改成「用使用者提問的那個語言回答」之後仍然無效，於是做了兩組實測：

1. **擺放位置**（`scripts/bench_reply_language.py`）：語言指令放在
   system 最後／system 最前／獨立 system 訊息／併進使用者提問 —— 四種 × 2 次
   → **8/8 全部用中文回答**。位置無關。
2. **上下文語言**：把整段檢索上下文換成**全英文**，英文提問 → **仍然回中文**。
   所以「模型跟著上下文語言走」這個假設是**錯的**（那是我原本的推測）。

→ 結論：**這是模型能力的限制，提示詞救不了。**

**對照組 `llama3`（已經在機器上，不用下載）**。擁有者要求「多試幾遍確認數據」，
所以加大樣本重跑 —— **而且結論因此改變了一次**，見下方的更正。
...
忠實度（挑「程式實驗室閒置多久」這一題 —— 知識庫寫得最明確、三種身分都有，
最容易看出模型是照抄還是自己補；各 **10 次**）：

| | 講了假話 | 完整（30＋120） | 簡體字總數 |
|---|---|---|---|
| `qwen2.5:7b` | **2 / 10** | 7 / 10 | **52** |
| `llama3` | **0 / 10** | 9 / 10 | **2** |

qwen 的假話是「**教師沒有閒置時間限制**」—— 知識庫寫的是 120 分鐘。
另外還出現過一次夾雜韓日文的亂碼輸出（`-Day 누진さんの使い方を教えてください。`）。
llama3 沒有講錯過任何一句；它唯一的「不合格」是**答不全**（漏講教師的 120 分鐘）。
**錯誤的性質不同**：qwen 斷言錯的事，llama3 只是講得不夠 —— 前者危險得多，
因為使用者不會知道那句是錯的。

llama3 還會**保留中文按鈕名再加英譯**（`Go to the "交給平台訓練" (Train with Platform) page`）。
代價：會加 😊、偶爾語句不順（如「進所實驗室」）。

⚠️ **更正：我第一次報的忠實度數字是誤導的。** 我用「每題 3 次」量到
qwen 15/15、llama3 14/15，據此說「qwen 內容比較可靠」——
但同一題加大到 10 次之後，qwen 是 7/10 且有 2 次講假話。
**3 次的樣本分不出「這個模型可靠」與「這三次剛好對」。**

**待決**：要不要把 `RAG_CHAT_MODEL` 從 `qwen2.5:7b` 換成 `llama3`？
那會影響**每一個**使用者，所以沒有自行更動。

**2026-08-28 已換（擁有者裁定）：`RAG_CHAT_MODEL` = `llama3`。**

順帶好處：`llama3` 本來就要為了 chat／簡報而 pull，所以**本機模型從 3 個變 2 個**，
少佔約 4.7 GB 顯示記憶體 —— 對部署指南 §1 那條「8GB GPU 載入模型後幾乎佔滿」的警語是好事。

⚠️ **泛指沒有用，一定要點名語言。** 換模型之後日文與越南文提問**被回成英文**。
實測：同一題說「用使用者提問的那個語言回答」→ 日文 **0/2**；
改成「Reply in **Japanese**」→ **2/2**。
所以 `language_directive()` 改成先 `detect_named_language()` 依**書寫系統**點名
（日／韓／泰／俄／阿拉伯／印地／越南／中文），認不出來的（英文與其他拉丁語系）才退回泛指。
判定函式 12 個情境全對（含「假名要先於漢字判」這個順序陷阱、簡體、空字串）。

**端對端複驗（走真實 `/assistant/ask`）**：中文／英文／日文／韓文／越南文 **五種都回對語言**。

⚠️ **品質隨語言遞減，這點要誠實記著**：
- 中文、英文：乾淨。中文 0 簡體字。
- 日文、韓文：可用，但會夾雜外語詞（出現過 `eller`、`訓练`、`進行中`）。
- 越南文：內容正確，四次裡有一次冒出造字（`huannabinating`），另有 `soubor` 這種夾雜。
- 偶爾會把中文頁名翻掉（提示詞要求保留原文加註解，模型沒完全照做）。

⚠️ **我在這一段兩次被 n=1 誤導，方向相反**：先用 3 次的樣本說「qwen 忠實度比較好」，
後來又用 1 次的樣本說「越南文會造字」。加大樣本後兩個結論都翻掉。
**小樣本會往兩個方向騙人，不是只會往壞的方向。**


**已經做了的**：`language_directive()` 依提問語言產生硬指令（客服與家教兩種模式都接上）。
對中文提問有用（釘住繁體）；對 qwen 的英文提問**沒有作用**，
這件事已寫進該函式的 docstring —— 不寫的話下一個人會以為它有效。

⚠️ **我在這一段犯的錯（記著）**：
1. 我在 `rag_service.py` 的註解裡寫「放前面會被知識區塊淹掉（**實測過**）」——
   **那是假的，我沒測過**，是把推測寫成觀察。已改掉，並補上真的實測腳本。
2. 簡體字偵測器第一版把「面」「記」「錄」這種**繁簡共用**的字列進去，
   量到一堆假陽性，差點據此比較兩個模型。修好尺之後結論反轉：
   llama3 的簡體漏出其實**比 qwen 少**。**量之前先驗尺。**
