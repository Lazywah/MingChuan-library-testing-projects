...
2. 🔴 使用者端與管理端**共用同一個 cookie** → 在一邊登入會讓另一邊安靜地失去權限
   （這也是為什麼管理端的功能面我沒測到 —— 我的 session 被蓋掉了，而重新登入要密碼）。

**我在稽核中判斷錯了三次**，都寫在流水帳裡（T5 假發現兩則、T21 更正兩則）。
共通點是**從設定或程式碼推論行為，而沒有去看它實際的日誌**。
...
**統計**：✅ 可用 **23** 項 ｜ ⚠️ 有保留 5 項 ｜ 🔴 已知問題 1 項 ｜ ⛔ 環境所限未測 **10** 項
