...
**我在稽核中判斷錯了四次**，都寫在流水帳裡（T5 兩則、T21 兩則、T23 一則）。
...
**兩個殘留的清理建議**（小、獨立）：
1. `admin_hud_token` 這個 localStorage 鍵已經沒有人用了，卻留著一個過期 6 天的值 —— 應該清掉，它只會誤導（我就被誤導了）。
2. 前端偵測到 token 過期時應該主動清掉並提示重新登入，而不是留著讓後續請求安靜地退回 cookie。
