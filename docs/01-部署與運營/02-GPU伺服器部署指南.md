...
由於訓練腳本與資料集存放於主機端 (Ubuntu)，GPU 伺服器 (Windows) 必須能夠存取這些檔案。我們強烈建議使用 **Samba (SMB) 網路芳鄰** 來實作跨系統目錄共享。

#### 方案說明與設定方式：

**A. 在主機端 (Ubuntu 服務層) 設定 Samba 共用：**
1. 安裝 Samba 服務：
   ```bash
   sudo apt update && sudo apt install samba -y
   ```
2. 編輯 Samba 設定檔 (`sudo nano /etc/samba/smb.conf`)，在檔案最下方加入您的資料目錄設定（假設專案資料皆存放於 `/opt/ai-platform/data`）：
   ```ini
   [ai-data]
   path = /opt/ai-platform/data
   available = yes
   read only = no
   browsable = yes
   public = no
   writable = yes
   ```
   *(註：請確保 `path` 所指的目錄中包含 `scripts`, `datasets`, 與 `outputs` 等子資料夾)*
3. 為 Samba 建立連線密碼，並重啟服務：
   ```bash
   sudo smbpasswd -a <您的Ubuntu使用者名稱>
   sudo systemctl restart smbd
   ```

**B. 在 GPU 伺服器 (Windows 11) 掛載網路磁碟：**
1. 開啟「檔案總管」，在左側「本機」點擊右鍵，選擇「**連線網路磁碟機 (Map network drive)**」。
2. 指定磁碟機代號 (例如 `Z:`)。
3. 在資料夾路徑中輸入：`\\<Ubuntu主機IP>\ai-data` (例如 `\\192.168.1.50\ai-data`)。
4. 勾選「**使用不同認證連線**」，點選完成後，輸入剛剛在 Ubuntu 設定的 Samba 帳號與密碼，並勾選記住密碼。
5. 連線成功後，開啟 `gpu-worker/docker-compose.yml`，確保 `STORAGE_MOUNT_PATH` 指向您剛剛設定的磁碟機路徑 (例如 `STORAGE_MOUNT_PATH=Z:\`)。
...
- Worker 定期偵測本地端 GPU 使用率 (低於 10% 視為空閒) + busy-GPU 追蹤集（防止 docker pull 期間雙重派發）
- 若有空閒 GPU，Worker 領取任務，並在本地動態下達 `docker run` 指令啟動訓練容器
- 支援三種執行模式：
  - **VS Code 「Run on GPU」**（`inline_code`）：使用者在 code-server 內右鍵 Run，aibase-runner extension 將檔案 / cell 編譯為 shell script，掛入容器以 `bash -eu run.sh` 執行
  - **自訂入口**（`entry_args`）：llama.cpp / vLLM 等非 Python 工具，直接傳入執行指令陣列
  - **傳統模式**（`script_path`）：執行指定路徑的 Python 腳本
- 支援 v2.0 學期鎖定 base images：
  - `aibase/pytorch:2026-spring` （CUDA 12.8 + PyTorch 2.7）
  - `aibase/pytorch-legacy:2026-spring` （CUDA 11.8 + PyTorch 2.3）
  - `aibase/tensorflow:2026-spring`
  - `aibase/huggingface:2026-spring` （transformers + diffusers + peft + bitsandbytes）
  - `aibase/llamacpp:2026-spring`
  - `aibase/vllm:2026-spring`
  - `aibase/dev-tools:2026-spring` （Python / C++ / CUDA / Java）
- v2.0 自動掛載：
  - per-user volume `home_{user_id}:/home/coder`（pip --user 套件、HF cache、使用者檔案跨次保留）
  - 共享 `shared_models:/opt/models:ro` （預下載熱門模型）
- v2.0 自動注入：使用者在設定頁存的 secrets（HF_TOKEN / WANDB_API_KEY 等）以 `-e KEY=VAL` 傳入容器；log 中 mask 為 `ab****yz` 避免洩漏
- 即時解析訓練日誌中的進度百分比並回報前端（支援 Epoch N/M、step N/M、[N/M]、Progress X%）
- 任務完成後容器自動銷毀 (`--rm`)，volume 內檔案保留
...
docker run --rm --gpus all pytorch/pytorch:2.7.0-cuda12.8-cudnn9-runtime python -c "import torch; print(torch.cuda.is_available(), torch.cuda.device_count())"
