...
<https://ai.lib.mcu.edu.tw:8888/> → 用 admin 帳號登入。**80% 維運工作從這裡做完，不需 SSH**。
（是 **https** 且用正式主機名 —— 8888 自 2026-09-01 起強制 TLS。）
...
> **數據頁可依學院／學系／行政單位分組（v3.8，#13）**：管理端「數據 → 平台使用」
> 右上角的「分組方式」下拉。學院**不是欄位**，由 `users.department` 外連
> `org_departments` 推出來 —— 改對照表就全站生效，不必回填。
>
> 🔴 用的是**外連不是內連**：對不到對照表的人（舊系名、打錯字、沒填）必須留下來，
> 否則他們會從統計裡**安靜地消失** —— 人數對不上，而畫面上看不出少了誰。
> 對不到的一律歸「未分類」。
>
> ⚠️ 依行政單位分組時，學生會全部落在「未分類」（`unit` 只有職員有值）。那是正確的。
> ⚠️ MYAI 消費分析（外部 AI）那一頁**還沒有**這個切換，它有自己的七分頁匯出，另外處理。

> **個人組織資料的一次性解鎖（v3.8）**：完整路徑是
> 使用者用「問題回報」申請 → 管理端**帳號 → 點該帳號 → 「修改個人資料的授權」**卡片核可
> → 使用者下次開任一頁時彈窗自動出現（只顯示核可範圍內的欄位、帶入現有值）
> → 存檔一次後自動上鎖。
>
> ⚠️ `/auth/me` 的 `profile_unlock` 是這條路的關鍵：前端原本只在 `onboarded_at` 為 null 時
> 才顯示彈窗，所以**少了這個欄位，管理者開了解鎖使用者端也打不開表單**（開發中實際發生過）。
>
> 管理端可「收回」還沒用掉的解鎖 —— 那是**標成已用掉，不刪紀錄**：
> 刪掉的話「這個人曾經被開過一次」就查不到了，而「開過但沒用」正是稽核想知道的事。
>
> **個人組織資料的一次性解鎖（v3.8）**：校區／學系／行政單位在初次設定之後**上鎖**。
> 那三項是分組統計的基礎 —— 讓人隨時自己改，報表會隨著人改資料而變動，而且看不出是誰改的。
>
> **使用者要改**：用既有的「問題回報」送單說明（刻意不另做申請單 ——
> 多一套只會讓使用者有兩個地方可送、管理者有兩個地方要看）。
> **管理者核可**：`POST /api/v1/admin/users/{id}/profile-unlock`，指定可改哪幾個欄位。
>
> 🔴 **「一次」＝使用者成功存檔一次，不是一段時間。** 時間窗沒有人會記得回來收，
> 而「長期開著的一次性權限」比不上鎖還糟：大家以為它是鎖著的。
> 填錯被擋**不會**浪費掉那一次。
>
> 🔴 **可解鎖的欄位永遠不含 `role` / `is_admin` / `is_active` / `email` / `password`**，
> `crud` 匯入時有自檢擋著。這個機制不能變成繞過型別層那道防線的後門。
>
> ⚠️ 連帶改動：`department` 已從使用者端的 `UserUpdate` schema **移除**。
> 在此之前任何人都能用 `PUT /auth/me` 把學系改成任何自由文字 ——
> 那既繞過組織對照表的驗證（打錯的系名會長成一個新的統計類別），也讓這個鎖形同虛設。
